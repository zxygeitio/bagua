    ...
    <article className="mx-auto max-w-5xl px-4 py-8 md:px-6">
      <div className="flex flex-wrap items-end justify-between gap-6">
        <div className="enter-up">
          ...
          <p className="mt-2 font-body text-sm tracking-[0.18em] text-bagua-muted">
            {gua.pronunciation}
          </p>
        </div>
      ...
      <div className="mt-12 grid gap-10 lg:grid-cols-[minmax(0,1fr)_14rem] cv-auto cis-m">
      ...
      <section className="mt-16 cv-auto cis-m">
        ...
        <ol className="space-y-6">
          {[...gua.yaos].map((yao) => {
            ...
            const label =
              yao.yinYang === 'yang'
                ? yaoLabelYang[yao.position - 1]!
                : yaoLabelYin[yao.position - 1]!
            return (
              <li key={yao.position} className="border-l-4 border-bagua-text pl-4">
                <p className="font-display text-xs tracking-[0.2em] text-bagua-primary">
                  {label}
                  <span className="ml-2 text-bagua-muted">
        ...
        {gua.yong ? (
          <div className="mt-6 border-l-4 border-bagua-primary pl-4">
            <p className="font-display text-xs tracking-[0.2em] text-bagua-primary">
              {gua.yong.label}
              <span className="ml-2 text-bagua-muted">通爻</span>
            </p>
            <p className="prose-classical mt-2 text-[18px]">{gua.yong.text}</p>
            <p className="prose-body mt-1 text-sm text-bagua-muted">
              《象》曰：{gua.yong.xiangZhuan}
            </p>
          </div>
        ) : null}
      </section>
...
function RelationMini({ label, sub, gua, disabled }: RelationMiniProps) {
  ...
  return (
    <Link
      ...
      prefetch={false}
      className="btn-press block border-4 border-bagua-text bg-bagua-surface p-3 hover:bg-bagua-wash"
    >

  ...
  const activeGua = (() => {
    switch (activeTab) {
      ...
      case 'bian':
        return bianGua
      case 'hu':
        return record.huGuaId ? getGuaById(record.huGuaId) : null
      case 'dui':
        return benGua ? getGuaById(benGua.duiGua) : null
      case 'zong':
        return benGua ? getGuaById(benGua.zongGua) : null
    }
  })()
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 md:px-6 md:py-12">
        ...
        <header className="enter-up mb-8 flex flex-wrap items-center justify-between gap-3 border-b-4 border-bagua-text pb-4">
          <div>
            ...
            <p className="mt-1 font-body text-xs text-bagua-text">
              {date} · {methodLabel}
            </p>
          </div>
          <div className="flex items-center gap-1.5">
            ...
            <IconBtn onClick={() => setShowShareCard(true)} label="下载卦象卡">
          ...
          <div className="enter-up stagger-1 mb-8 grid gap-3">
            ...
            {record.scenario && (
              <div className="flex flex-wrap items-center gap-2 text-xs">
                ...
                {changingCount > 0 && (
                  <span className="ml-auto font-body text-bagua-muted">{changingCount} 爻动</span>
                )}
              </div>
            )}
          </div>
        ...
        {reading && (
          <Reveal>
            <section className="mb-10">
              <SectionHeader index="01" title="朱熹判读" desc={reading.rule.explain} />
              ...
              {activeGua ? (
                <div className="bg-bagua-canvas/30 p-8 text-center">
                  ...
                  <p className="mt-2 font-body text-sm text-bagua-muted">
                    #{activeGua.id.toString().padStart(2, '0')} / 64 · {activeGua.pronunciation} ·
                    五行属{activeGua.wuxing}
                  </p>
                </div>
              ) : (
                    ...
                    <Reveal key={yao.position} delay={i * 60} direction="left">
                      ...
                      >
                        <p className="flex items-center gap-2 font-display text-xs tracking-widest">
                          <span
                            className={
                              isChanging || isPrimary ? 'text-bagua-primary' : 'text-bagua-text'
                            }
                          >
                            {label}
                          </span>

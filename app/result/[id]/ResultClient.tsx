    ...
    <main className="min-h-screen bg-bagua-canvas">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`rounded-full p-2 transition ${
                record.favorite
                  ? 'bg-bagua-accent/10 text-bagua-accent'
                  : 'text-bagua-muted hover:bg-bagua-border/30'
              }`}
      ...
      <section className="container mx-auto max-w-4xl px-6 py-8">
        ...
        <div className="mb-6 text-center">
          ...
          {record.question && (
            <p className="mt-2 font-calligraphy text-lg text-bagua-text">
              问：{record.question}
            </p>
          )}
        </div>
        ...
        <div className="mb-8 rounded-card border border-bagua-border/40 bg-bagua-surface shadow-md">
          <div className="flex overflow-x-auto border-b border-bagua-border/30">
            {(['ben', 'bian', 'hu', 'dui', 'zong'] as Tab[]).map(tab => (
              <button
                ...
                className={`flex-1 whitespace-nowrap px-6 py-3 font-display text-sm font-medium transition ${
                  activeTab === tab
                    ? 'border-b-2 border-bagua-primary text-bagua-primary'
          ...
          {activeGua ? (
            <div className="p-8 text-center">
              ...
              <h2 className="font-calligraphy text-3xl font-bold text-bagua-text">
                {activeGua.name}
              </h2>
              <p className="mt-2 font-body text-sm text-bagua-muted">
                #{activeGua.id} / 64 · {activeGua.pronunciation} · 五行属{activeGua.wuxing}
              </p>
            </div>
          ) : (
        ...
        {activeGua && (
          <div className="space-y-6">
            ...
            <div className="rounded-card border border-bagua-secondary/40 bg-gradient-to-br from-bagua-secondary/5 to-bagua-surface p-6 shadow-md">
              ...
              <p className="font-body leading-relaxed text-bagua-text">
                {activeGua.modernInsight}
              </p>
            </div>
          </div>
        )}
            ...
            <div className="space-y-4">
              {[...benGua.yaos].reverse().map((yao, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`rounded-card border-l-4 pl-4 ${
                      isChanging
                        ? 'border-l-bagua-accent bg-bagua-accent/5'
                        : 'border-l-bagua-border/40'
                    }`}
                  >
                    <div className="mb-1 flex items-center gap-2">
                      <span className={`font-display text-base font-bold ${isChanging ? 'text-bagua-accent' : 'text-bagua-primary'}`}>
                        {label}
                      </span>
                      {isChanging && (
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          <Link
            href="/divine"
            className="inline-flex items-center gap-2 rounded-button border border-bagua-primary bg-bagua-primary px-6 py-3 font-body font-medium text-white transition hover:bg-bagua-primary/90"
          >
          ...
          <button
            onClick={() => {
              if (navigator.share) {
                navigator.share({
                  title: `bagua · ${benGua?.name}`,
                  text: `我刚起了一卦：${benGua?.name}`,

        ...
        >
          ...
          >
            ...
            <p className="mt-2 font-body text-sm text-bagua-muted">
              先按 <kbd className="border border-bagua-text bg-bagua-wash px-1.5 py-0.5 font-mono text-xs">g</kbd>，
              再按第二键。0.8 秒内有效。
            </p>
            <ul className="mt-5 space-y-2">
              {Object.entries(SHORTCUTS).map(([key, s]) => (
                <li key={key} className="flex items-center justify-between border-b border-bagua-fiber/40 py-1.5">
                  <Link
                    href={s.path}
                    onClick={() => setOpen(false)}
                  ...
                  <span className="font-mono text-xs text-bagua-muted">
                    {s.key.split(' ').map((k, i) => (
                      <span key={i}>
                        <kbd className="border border-bagua-text bg-bagua-wash px-1.5 py-0.5">{k}</kbd>
                        {i === 0 && ' '}
                      </span>
                    ))}
                  </span>
            ...
            <p className="mt-5 text-center font-body text-xs text-bagua-muted">
              <kbd className="border border-bagua-text bg-bagua-wash px-1.5 py-0.5 font-mono">?</kbd>{' '}
              随时打开此面板
            </p>
          </div>
        </div>
